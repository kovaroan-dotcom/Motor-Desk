...
def main():
    ...
    world  = diverse_limit(sorted([a for a in all_articles if a['region']=='world'],  key=lambda a:a['ts'],reverse=True), 100, 8)
    europe = diverse_limit(sorted([a for a in all_articles if a['region']=='europe'], key=lambda a:a['ts'],reverse=True),  80, 8)
    china  = diverse_limit(sorted([a for a in all_articles if a['region']=='china'],  key=lambda a:a['ts'],reverse=True),  60, 6)
    cz     = diverse_limit(sorted([a for a in all_articles if a['region']=='cz'],     key=lambda a:a['ts'],reverse=True),  60, 8)
