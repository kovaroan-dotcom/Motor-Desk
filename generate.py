...
def main():
    ...
    def section_html(articles: list[dict], flag: str, label: str, hero: bool = False, section_class: str = "") -> str:
        ...
        if hero and len(articles) > 0:
            ...
            rest_grid = f'<div class="grid">{"".join(card_html(a,"s4") for a in articles[3:6])}{"".join(card_html(a,"s3") for a in articles[6:])}</div>' if articles[3:] else ""
            return f"""
      <div class="rsec section-wrap{extra_class}" data-region="{slug}">
        <div class="rh">
    ...
    ctx_world_sec = section_html(ctx_world, "🌍", "World News",    hero=True,  section_class="ctx-section")
    ctx_eu_sec    = section_html(ctx_eu,    "🇪🇺", "Europe & EU",  hero=False, section_class="ctx-section")
    ctx_cz_sec    = section_html(ctx_cz,    "🇨🇿", "Czech Republic News", hero=False, section_class="ctx-section")
