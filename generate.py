...
def generate_brief(automotive_articles: list, world_articles: list, is_morning: bool) -> str:
    ...
    def item_html(a):
        ...
        return (f'<div class="brief-item"><div class="brief-item-title">{title}</div><div class="brief-item-summary">{summ}</div><div class="brief-item-meta"><span class="brief-item-src">{src}</span><span>{date}</span>{link_tag}</div></div>\n')
    ...
    selection_note = "Výběr: 5 nejdůležitějších zpráv z každé sekce za posledních 24h, seřazených podle dopadu na Škoda Auto (Ohrožení &rarr; Příležitost &rarr; Sledovat &rarr; Info)."
    return (f'<div class="brief-intro">{selection_note}</div>'
            '<div class="brief-section-title">🚗 Automotive</div>'
            + auto_html +
            '<div class="brief-section-title" style="margin-top:8px">🌍 Svět &amp; Kontext</div>'
            + world_html)
