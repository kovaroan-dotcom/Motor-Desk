...
def generate_monthly_digest(all_articles: list) -> str:
    ...
    for i, a in enumerate(top):
        em = sig_emoji.get(a.get("signal","info"),"🔵")
        url = a.get("url","")
        title = html.escape(a.get("title","")[:100])
        linked = f'<a href="{html.escape(url)}" target="_blank" style="color:inherit">{title}</a>' if url else title
        src = html.escape(a.get("source",""))
        date = html.escape(a.get("date",""))
        rows.append(
            f'<tr style="border-bottom:1px solid #e9ecef">'
            f'<td style="padding:10px 8px;font-size:12px;color:#868e96">{i+1}</td>'
            f'<td style="padding:10px 8px">{em}</td>'
            f'<td style="padding:10px 8px;font-size:13px;font-weight:600">{linked}</td>'
            f'<td style="padding:10px 8px;font-size:11px;color:#868e96">{src}</td>'
            f'<td style="padding:10px 8px;font-size:11px;color:#868e96">{date}</td>'
            f'</tr>'
        )

    stat_box = lambda bg, val, lbl, col: (
        f'<div style="background:{bg};border-radius:8px;padding:14px 18px;flex:1;min-width:110px;text-align:center">'
        f'<div style="font-size:28px;font-weight:900;color:{col}">{val}</div>'
        f'<div style="font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:#868e96">{lbl}</div></div>'
    )
    stats = (
        stat_box("#f8f9fa", len(all_articles), "Článků", "#0e3a2f") +
        stat_box("#fff0f0", signals.get("threat",0), "Rizik", "#b91c1c") +
        stat_box("#f0fff8", signals.get("opportunity",0), "Příležitostí", "#0e3a2f") +
        stat_box("#fff8e0", signals.get("watch",0), "Sledovat", "#b45309")
    )
    return (
        ...
        '<thead><tr style="border-bottom:2px solid #0e3a2f">'
        '<th style="padding:8px;text-align:left;font-size:9px;color:#868e96">#</th>'
        '<th style="padding:8px"></th>'
        '<th style="padding:8px;text-align:left;font-size:9px;color:#868e96">Titulek</th>'
        '<th style="padding:8px;text-align:left;font-size:9px;color:#868e96">Zdroj</th>'
...
def generate_brief(automotive_articles: list, world_articles: list, is_morning: bool) -> str:
    ...
    def ai_select_top5(articles: list, section: str) -> list:
        ...
        prompt = (f"You are selecting the top 5 most important news articles for Škoda Auto's {section} briefing. "
                  ...
                  f"STRICTLY EXCLUDE (do not select under any circumstances): crime, police, missing persons, sports scores, sports results, celebrity gossip, TV series, streaming reviews, movie reviews, entertainment news, local accidents, weather, human interest stories, obituaries, cultural events, concerts. "
                  f"ONLY SELECT: business, economy, trade policy, regulation, energy policy, technology breakthroughs, geopolitics, financial markets, automotive industry, AI/tech with business impact. "
    ...
    def item_html(a):
        ...
        raw_desc = a.get('desc') or ''
        if len(raw_desc) > 220:
            cut = raw_desc[:220]
            last_period = cut.rfind('. ')
            last_space = cut.rfind(' ')
            if last_period > 100:
                raw_desc = cut[:last_period+1]
            elif last_space > 0:
                raw_desc = cut[:last_space] + '…'
            else:
                raw_desc = cut + '…'
        summ  = html.escape(raw_desc)
        src   = html.escape(a.get('source',''))
        date  = html.escape(a.get('date',''))
        url   = a.get('url','')
...
(a Czech automaker, VW Group member, manufacturing in Czech Republic, exporting across Europe/China/India).
...
- threat: creates real risk for automotive business (tariffs, sanctions, energy crisis, recession signals, supply chain disruption)
- opportunity: creates favorable conditions (new trade deals, subsidies, infrastructure investment, friendly regulation)
...
SOURCE_REPUTATION = {
    "Reuters Autos": 98, "BBC News": 97, "Associated Press": 97, "Financial Times": 96,
    "Politico": 92, "Politico EU Autos": 92, "Politico Europe": 92, "Der Spiegel": 93,
    "Wired": 88, "Euronews": 88, "TechCrunch Transport": 85, "The Verge": 85,
    "Transport & Env.": 87, "Autovista24": 86, "Automotive News": 90,
    "Electrek": 80, "InsideEVs": 80, "CleanTechnica": 78, "Electrive": 82,
    "Green Car Reports": 78, "Motor1": 76, "The Drive": 76, "Autocar": 84,
    "Auto Express": 80, "CnEVPost": 75, "CarNewsChina": 74, "Gasgoo": 78,
    "Hybrid.cz": 78, "Auto.cz": 76, "AutoRevue.cz": 74, "iDnes Auto": 82,
    "Autoforum.cz": 72, "EV Magazin CZ": 74, "iRozhlas": 94, "ČT24": 93,
    "Seznam Zprávy": 86, "Hospodářské noviny": 90, "Deník N": 88, "CzechCrunch": 80,
}
...
def fetch_feed(source: dict, month_start: datetime, month_end: datetime) -> list[dict]:
    ...
    for item in items:
        ...
        # Entertainment/streaming/TV series detection
        ENTERTAINMENT_KW = ["serial","seriál","streaming","netflix","hbo","disney+","prime video",
                            "tv series","sezóna","season premiere","recenze filmu","film review",
                            "celebrity","celebrita","herečka","herec","zpěvák","zpěvačka","koncert",
                            "album","reality show","talent show","soutěž krásy"]
        is_entertainment = any(k in content_lower for k in ENTERTAINMENT_KW)
        is_ctx_region = source["region"] in ("ctx-cz", "ctx-eu", "ctx-world")

        if is_entertainment:
            if is_ctx_region:
                cat = "culture"  # allowed only in World&Context -> Culture
            else:
                continue  # skip entertainment articles entirely in automotive/other sections
...
def main():
    ...
    world  = diverse_limit(sorted([a for a in display_articles if a['region']=='world' and not a.get('is_pr')],  key=lambda a:a['ts'],reverse=True), 100, 8)
    pr     = sorted([a for a in display_articles if a.get('is_pr')], key=lambda a:a['ts'], reverse=True)
    ...
    def card_html(a: dict, size: str) -> str:
        ...
        pr_src = next((s for s in SOURCES if s["name"]==a.get("source","") and s.get("is_pr")), None)
        pr_badge = '<span class="badge badge-pr">📋 OFFICIAL</span>' if pr_src else ""
        badges = f'<div class="badge-wrap">{breaking_badge}{new_badge}{must_badge}{paywall_badge}{pr_badge}</div>' if (is_breaking or is_new_only or is_must or paywall_src or pr_src) else ""
    ...
    def section_html(articles: list[dict], flag: str, label: str, hero: bool = False, section_class: str = "") -> str:
        ...
        slug_map = {"Global":"world","World":"world","Europe":"europe","China":"china",
                    ...
                    "Czech Republic News":"ctx-cz","Tiskové zprávy":"pr"}
    ...
    pr_section     = section_html(pr,     "📋", "Tiskové zprávy", section_class="pr-section") if pr else ""
    ...
    content = world_section + europe_section + china_section + cz_section + pr_section
    ...
    output = (
        ...
        .replace("{{MONTHLY_CONTENT}}", generate_monthly_digest(all_articles) if is_month_end else "")
