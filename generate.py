...
def generate_brief(automotive_articles: list, world_articles: list, is_morning: bool) -> str:
    ...
    def ai_select_top5(articles: list, section: str) -> list:
        ...
        prompt = (f"You are selecting the top 5 most important news articles for Škoda Auto's {section} briefing. "
                  f"The audience is Škoda Auto management and employees.\n\n"
                  f"Articles:\n{article_list}\n\n"
                  f"Select the 5 most newsworthy and business-relevant articles. "
                  f"EXCLUDE: crime, missing persons, celebrity gossip, sports scores, entertainment reviews, local accidents. "
                  f"INCLUDE: business, economy, politics, technology, automotive industry, energy, international relations. "
                  f"Respond with ONLY a comma-separated list of numbers, e.g.: 2,5,1,8,3")
        try:
            req_data = json.dumps({
