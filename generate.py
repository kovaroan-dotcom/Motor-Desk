...
def generate_brief(automotive_articles: list, world_articles: list, is_morning: bool) -> str:
    ...
    auto_top  = sorted(automotive_articles, key=lambda a: sig_order.get(a.get("signal","info"), 3))[:5]
    world_top = sorted(world_articles,      key=lambda a: sig_order.get(a.get("signal","info"), 3))[:5]
    ...
    def item_html(a):
        ...
        url   = a.get('url','')
        img   = a.get('img','')
        img_html = f'<img src="{html.escape(img)}" alt="" style="width:100%;aspect-ratio:16/9;object-fit:cover;display:block">' if img else ''
        link  = f'<a class="brief-read" href="{html.escape(url)}" target="_blank" rel="noopener">Číst &#8594;</a>' if url else ''
        return ('<div class="brief-item">' + img_html +
                '<div style="padding:16px 20px 14px">'
                f'<div class="brief-item-title">{title}</div>'
                f'<div class="brief-item-summary">{summ}</div>'
                f'<div class="brief-item-meta"><span class="brief-item-src">{src}</span>'
                f'<span>{date}</span>{link}</div></div></div>\n')
    ...
    return (f'<div class="brief-time">{brief_type} · aktualizováno {time_label}</div>'
            f'<div class="brief-section-title">🚗 Automotive — top 5 za posledních 24h</div>'
            + auto_html +
            '<div class="brief-section-title" style="margin-top:24px">🌍 Svět &amp; Kontext — top 5 za posledních 24h</div>'
            + world_html)
...
SOURCES = [
    ...
    {"name": "Financial Times",  "url": "https://www.ft.com/rss/home",                                             "region": "ctx-world","cat": "economy"},
...
def main():
    ...
    def card_html(a: dict, size: str) -> str:
        ...
        badges = f'<div class="badge-wrap">{breaking_badge}{new_badge}{must_badge}</div>' if (is_breaking or is_new_only or is_must) else ""
    ...
    ctx_world  = [a for a in all_articles if a["region"] == "ctx-world"]
    ctx_eu     = [a for a in all_articles if a["region"] == "ctx-eu"]
    ctx_cz     = [a for a in all_articles if a["region"] == "ctx-cz"]
